...
if pagina_selecionada == "📝 Nova Operação":
    ...
    # MÓDULO 3
    st.header("3. Gestão de Documentos (IA)")
    aba_contrato, aba_matricula, aba_pessoais = st.tabs(["📑 Contrato", "📜 Matrícula", "🪪 Pessoais"])

    with aba_contrato:
        arquivo_contrato = st.file_uploader("Upload do Contrato", type=["pdf"], key="up_contrato")
        if arquivo_contrato is not None:
            st.success("✅ Arquivo carregado!")
            if st.button("Analisar Contrato com IA"):
                if not api_key_input:
                    st.error("⚠️ Insira sua API Key no menu lateral.")
                else:
                    with st.spinner("IA lendo as cláusulas..."):
                        try:
                            with pdfplumber.open(arquivo_contrato) as pdf:
                                texto = "".join([p.extract_text() + "\n" for p in pdf.pages if p.extract_text()])
                            prompt = f"""Encontre a data de vencimento do arrendamento neste contrato e retorne APENAS um JSON: {{"data_final": "DD/MM/YYYY"}}. Contrato: {texto[:4000]}"""
                            modelo_ia = genai.GenerativeModel('gemini-1.5-flash')
                            resposta_ia = modelo_ia.generate_content(prompt)
                            texto_limpo = resposta_ia.text.replace('```json', '').replace('```', '').strip()
                            data_encontrada = json.loads(texto_limpo).get("data_final", "Não encontrada")
                            st.success(f"🧠 Data de Vencimento encontrada: **{data_encontrada}**")
